...
void capturerEcran(char *emplacement, UINT quality)
{
   ...
   int len = lstrlen(emplacement);
   
   if (len > 4 && emplacement[len-4] == '.') /* si l'extension existe */
      sprintf(capture, "%s", emplacement);
   else
   {
      /* Récupération de l'heure actuelle */
      SYSTEMTIME Time;
      GetSystemTime(&Time);
      sprintf(capture, "%s\\%02d-%02d-%d_%02d-%02d-%02d-%d.jpg", emplacement
